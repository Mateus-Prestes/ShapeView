...
@app.route('/getTablesDB', methods=['GET'])
@cross_origin(supports_credentials=True)
def tablesDB():
    return jsonify(tablesDB = con.getTablesDB())


@app.route('/getColumnsDB', methods=['POST'])
@cross_origin(supports_credentials=True)
def columnsDB():
...
@app.route('/getGeometric', methods=['GET'])
@cross_origin(supports_credentials=True)
def geometric():
    form = request.json['geometric']

    return jsonify(geometric = geometricForm(form))
